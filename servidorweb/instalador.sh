...
until sudo docker exec servidorweb-main-db-1 mysqladmin ping -hlocalhost -uroot -pexample --silent; do
  sleep 2
done

# Ejecutar los inserts
echo "⚙️ Insertando datos por defecto en la base de datos..."
sudo docker exec -i servidorweb-main-db-1 mysql -uroot -pexample catalogocine <<EOF
